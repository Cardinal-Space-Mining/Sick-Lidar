...
|-|-|-|
| [sick_scan_xd](https://github.com/SICKAG/sick_scan_xd) | Included as submodule, built using CMake `subdirectory` call from top level CMakeLists.txt | **Make sure to clone recursively** so that the submodule is pulled! |
...
- **To install the shared libraries, test program, and headers, use `cmake --install ./PATH/TO/BUILD/DIRECTORY`. The default install location is in the project directory under `install`, but this can be changed by modifying the cmake variable `CMAKE_INSTALL_PREFIX`.**
