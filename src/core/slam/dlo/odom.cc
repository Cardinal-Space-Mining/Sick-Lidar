...
void dlo::OdomNode::publishPose() {
  ...
  this->pose.header.stamp = this->scan_stamp;
  this->pose.header.frame_id = this->odom_frame;

  this->pose.pose.position.x = this->pose[0];
  this->pose.pose.position.y = this->pose[1];
  this->pose.pose.position.z = this->pose[2];

  this->pose.pose.orientation.w = this->rotq.w();
  this->pose.pose.orientation.x = this->rotq.x();
  this->pose.pose.orientation.y = this->rotq.y();
  this->pose.pose.orientation.z = this->rotq.z();

  this->pose_pub.publish(this->pose);
}
...
void dlo::OdomNode::publishKeyframe() {
  ...
  if (this->keyframe_cloud->points.size() == this->keyframe_cloud->width * this->keyframe_cloud->height) {
    ...
    pcl::toROSMsg(*this->keyframe_cloud, keyframe_cloud);
    keyframe_cloud.header.stamp = this->scan_stamp;
    keyframe_cloud.header.frame_id = this->odom_frame;
    this->keyframe_pub.publish(keyframe_cloud);
  }
...
void dlo::OdomNode::icpCB(const pcl::PointCloud& pc) {
  ...
  this->current_scan = pcl::PointCloud<PointType>::Ptr (&pc, [](auto*){});
...
void dlo::OdomNode::imuCB(const sensor_msgs::Imu::ConstPtr& imu) {
  ...
  if (!this->imu_calibrated) {
    ...
    while(imu_buffer.size() > imu_buffer_size_){
      imu_buffer.resize(imu_buffer_size_);
    }
    this->mtx_imu.unlock();
